...
public class WeightedGraph<V> extends AbstractGraph<V> {
  ...
  public ShortestPathTree getShortestPathByMatrix(int sourceVertex) {
   ...
   while (T.size() < numberOfVertices) {
    ...
    for (int u : T) {
     ...
     while (!isEmptyM(adjacencyMatrix, u) && T.contains(peekM(adjacencyMatrix, u).v)) {
    	 
      removeM(adjacencyMatrix, u);
      
     }

     if (isEmptyM(adjacencyMatrix, u)) {
     ...
     WeightedEdge e = peekM(adjacencyMatrix, u);
  ...
  private boolean isEmptyM(Double[][] adjacencyMatrix, int u) {
  ...
  private void removeM(Double[][] adjacencyMatrix, int u) {
	  
	  for (int i = adjacencyMatrix.length - 1; i >= 0; i--) {
	   
		  if(adjacencyMatrix[u][i] != null) {
    	
			  adjacencyMatrix[u][i] = null;
  ...
  private WeightedEdge peekM(Double[][] adjacencyMatrix, int u) {
	  
	  for (int i = adjacencyMatrix.length - 1; i >= 0; i--) {
	   
		  if(adjacencyMatrix[u][i] != null) {
    	
			  return new WeightedEdge(u, i, adjacencyMatrix[u][i]);
